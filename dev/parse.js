const txt = `-b, --bitrate : Set bitrate. Use bits per second (e.g. 10MBits/s would be -b 10000000)
...
-sg, --segment  : Segment output file in to multiple files at specified interval <number>
...
-x, --vectors : Output filename <string> for inline motion vectors
...
-r, --raw : Output filename <string> for raw video
...
-w, --width : Set image width <number>
-h, --height  : Set image height <number>
-o, --output  : Output filename <string> (to write to stdout, use '-o -'). If not specified, no file is saved
...
-p, --preview : Preview window settings <string>
...
txt.split('\n').forEach( l => {
	let name = l.split(' : ')[0]
	let desc = l.split(' : ')[1].trim()
	let type = 'boolean'
	if (desc.indexOf('<string>') != -1) type = 'string'
	if (desc.indexOf('<number>') != -1) type = 'number'
	name = name.substring( name.indexOf('--') + 2).trim()

	j.push( {name, desc, type} )
})

require('fs').writeFileSync('./camera.json', JSON.stringify(j))