...
let currentInterface = defaultInterface;
...
function connection(details, callback) {
  ...
  console.log('[pi-wifi] connection...')
  let netId;

  // TODO: check is a password is needed?

  let params = prepareConnectionDetails(details);
...
function detectSupplicant(callback) {
  exec(commands.detectSupplicant, function (err, stdout) {
    let iface, configFile;
    let result = false;
    if (!err) {
      let lines = stdout.split('\n');
      for (let i = 0; i < lines.length; i++) {
        if (lines[i].indexOf('wlan') > -1) {
          let options = lines[i].split(' ');
...
function startSupplicant(options, callback) {
  ...
  let iface = options.hasOwnProperty('iface') ? options.iface : currentInterface;
  let configFile = options.hasOwnProperty('config') ? options.config : defaultSupplicantConfigFile;
  let dnsFile = options.hasOwnProperty('dns') ? options.dns : defaultDNSFile;
...
function listInterfaces(callback) {
  exec(commands.wpaListInterfaces, function (err, stdout) {
    ...
    if (stdout) {
      ...
      let markerSeen = false // whether we've seen the marker yet
...
function listNetworks(callback) {
  exec(commands.wpaList, function (err, stdout) {
    let tempNetworkJson, parameters, networksArray;

    if (!err) {
      let networksList = stdout.split('\n');
      networksArray = [];
      networksList.splice(0, 2); //Remove headers
      networksList.splice(networksList.length - 1, 1); //Remove footer

      for (let networkIndex in networksList) {
        tempNetworkJson = {};
 ...
 * @description Used to replace preset letiables strings (e.g.: This is a :VAR text)
 * @param {string} text Text containg the letiables to be replaced
 * @param {Object} toReplace Json object that contains the string to find (key) and the replace string (value)
 * @return Returns the text after replacing the letiables
 */
